...
static int icount_start_insn_idx[2];
...
static inline void gen_tb_start(TranslationBlock *tb)
{
    ...
    if (tb_cflags(tb) & CF_USE_ICOUNT) {
        ...
        icount_start_insn_idx[0] = tcg_op_buf_count();
...
static inline void gen_tb_mid(TranslationBlock *tb)
{
    TCGv_i32 count, imm;
    if(!tcg_ctx->exitreqmid_label) {
        tcg_ctx->exitreqmid_label = gen_new_label();
    }
    ...
    if (tb_cflags(tb) & CF_USE_ICOUNT) {
         ...
         * of the movi so that we later (when we know the actual insn count)
         * can update the immediate argument with the actual insn count.  */
        icount_start_insn_idx[1] = tcg_op_buf_count();
        tcg_gen_movi_i32(imm, 0xdeadbeef);
    ...
    tcg_gen_brcondi_i32(TCG_COND_LT, count, 0, tcg_ctx->exitreqmid_label);
